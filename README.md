...
> Answer: It can be more performant because the page is not completely reloaded ever time a user interacts with it. Instead, individual components load or disappear. This speeds up the site after the initial load.
- [x] What does AJAX stand for?
> Answer: Asynchronous Javascript And XML. 
- [x] What are `controlled components` in React?
> Answer: These are components in which state from a DOM element such as <input> is controlled by React. This can be done for example with a useState hook. 
